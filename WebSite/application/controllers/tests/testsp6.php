...
class TestSP6 extends CI_Controller
{    
    ...
    public function index(){
      ...
      $this->getFilesByDir(0);
   	  $row = $this->getAllSCSCrossMethod($invocationId);      
      $row1 = $this->getAllSCSCrommMethodSecondaryTableRows($invocationId);   
    ...
    private function getFilesByDir($invocationId, $func='is_array'){
	  ...
	  private function getAllSCSCrossMethod($invocationId){
      $data = $this->scc->getSCSAcrossMethodPrimaryTable($invocationId);                
      $total = count($data);
      $row = array();
      if ($total > 0){
        $row = isset($data[0]) ? $data[0] : array();
      }
      $this->unit->run($total, 'is_array', "getAllSCSCrossMethod Test If User Has Any SCS Cross Method Rows With InvocationId[$invocationId]  Will Pass if If we have more than 0 rows");
      return $row;
    }
	
	private function getAllSCSCrommMethodSecondaryTableRows($invocationId){
	$row = $this->getAllSCSCrossMethod($invocationId);
	$secondary_table_rows = array();
	if ($row) {
          //foreach ($result as $row) {
	...
	$total = count($secondary_table_rows);
	if ($total > 0){
        $row = isset($data[0]) ? $data[0] : array();
      }
      $this->unit->run($total, 'is_array', "getAllSCSCrossMethodSecondaryTable Test If User Has Any SCS Cross Method Rows With InvocationId[$invocationId]  Will Pass if If we have more than 0 rows");
